...
module.exports = NodeHelper.create({
  ...
  updateWOTrend: async function (city, userAgent) {
    ...
    const WO_DAILY_URL = URL.parse(
      `https://api-app.wetteronline.de/app/weather/forecast?av=2&mv=13&c=d2ViOmFxcnhwWDR3ZWJDSlRuWeb=&location_id=${gid}&timezone=${process.env.TZ}`
    );
    let daily_promise = await this.getUrl(WO_DAILY_URL, userAgent);
    let dailyData = JSON.parse(await daily_promise);

    const WO_HOURLY_URL = URL.parse(
      `https://api-app.wetteronline.de/app/weather/hourcast?av=2&mv=13&c=d2ViOmFxcnhwWDR3ZWJDSlRuWeb=&location_id=${gid}&timezone=${process.env.TZ}`
    );
  ...
  extractEvent: function (dailyData, hourlyData, body) {
    ...
    let symbolUrlMatch = body.match(
      /<span class="daylabel">[^<]*<\/span>\s*<img src="([^"]+)\/[^/]+"/ms
    );
    let dailiesSymbolUrl = (hourliesSymbolUrl = symbolUrlMatch
      ? `${symbolUrlMatch[1]}/`
      : null);
    ...
    hourlyData["hours"].forEach((hourlyForecast) => {
      hourlies.push({
        ...
        wind_speed_kmh:
          hourlyForecast["wind"]["speed"]["kilometer_per_hour"]["value"]
      });
    });
    ...
    dailyData.forEach((dailyForecast) => {
      dailies.push({
        ...
        day_time_label: new Date(
          dailyForecast["sun"]["rise"]
        ).toLocaleDateString(new Intl.NumberFormat().resolvedOptions().locale, {
          weekday: "short"
        })
      });
    });
    ...
    let currentCondMatch = body.match(
      /WO\.metadata\.p_city_weather\.nowcastBarMetadata = (\{.+\})$/m
    );
    let firstHourlyMatch = body.match(
      /hourlyForecastElements\.push\((\{[^}]+\})/ms
    );
    let currConditions = {
      symbol_text: currentCondMatch
        ? JSON.parse(currentCondMatch[1])["nowcastBar"][0]["text"]
        : null,
      wind_speed_text: firstHourlyMatch
        ? this.parseInlineJson(firstHourlyMatch[1])["windSpeedText"]
        : null,
      wind_speed_kmh: firstHourlyMatch
        ? this.parseInlineJson(firstHourlyMatch[1])["windSpeedKmh"]
        : null
    };
