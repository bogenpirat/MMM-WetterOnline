...
Module.register("MMM-WetterOnline", {
  ...
  getDom: function () {
    ...
    if (Object.keys(this.weatherData).length > 0) {
      ...
      currentWrapper.insertAdjacentHTML(
        "beforeend",
        this.weatherData.currConditions.symbol_text
      );
      currentWrapper.insertAdjacentHTML("beforeend", "<br />");
      currentWrapper.insertAdjacentHTML(
        "beforeend",
      ...
      if (parseInt(this.config.daysTrend) > 0) {
        ...
        for (
          let i = 0;
          i < this.weatherData.dailies.length && i < this.config.daysTrend;
          i++
        ) {
          headerHtml += `<th class="trendcell">${this.weatherData.dailies[
            i
          ].day_time_label
            .replace(/ - .*/, "")
            .substring(0, 2)}</th>`;
